...
# ------------------------------------------------
# 1️⃣ LOAD MAIN CSV FROM GOOGLE DRIVE
# ------------------------------------------------
@st.cache_data
def load_data():
    url = "https://drive.google.com/uc?id=1M6I2ku_aWGkWz0GypktKXeRJPjNhlsM2"
    df = pd.read_csv(url)
...
# ------------------------------------------------
# 2️⃣ LOAD KERALA BOUNDARY (GeoJSON from GitHub)
# ------------------------------------------------
@st.cache_data
...
# ------------------------------------------------
# 3️⃣ CLIP POLLUTION POINTS TO KERALA BOUNDARY
# ------------------------------------------------
@st.cache_data
...
st.write(f"Displaying pollutant: **{pollutant}**")
...
# ------------------------------------------------
# 8️⃣ MONTHLY ANIMATION (FIXED SORTING)
# ------------------------------------------------
    ...
    df_month = (
        df_m.groupby(["ym_dt", "lat", "lon"])[pollutant]
        .mean()
        .reset_index()
        .sort_values("ym_dt")
    )
